...
class Experiment:
    ...
    def run(self, exp_num):
        ...
        plot_model(link_prediction_model, show_shapes=True, to_file=RESULT_PATH + str(exp_num)  + '/Link prediction model.png')

        x, y = data_arrange(data['train_1'], TOPOLOGICAL_FEATURE_NAMES, attrs=USE_ATTRIBUTES)
        link_prediction_model.fit(x, y, epochs=EPOCH) 

        feature_importance(link_prediction_model, TOPOLOGICAL_FEATURE_NAMES, embed_dim=EMBED_DIM, attrs=USE_ATTRIBUTES, path=RESULT_PATH + str(exp_num) + '/Feature importance of link prediction model.png')
        ...
        plot_model(classification_model, show_shapes=True, to_file=RESULT_PATH + str(exp_num) + '/Classification model.png')
        ...
        feature_importance(classification_model, TOPOLOGICAL_FEATURE_NAMES, embed_dim=EMBED_DIM, attrs=USE_ATTRIBUTES, path=RESULT_PATH + str(exp_num) + '/Feature importance of classification model.png')
        ...
        data['test_2']['true_abs_error'] = data['test_2'].apply(lambda row: math.fabs(row['goal']-row['predicted_link_probability']), axis=1)
        test_median_error = data['test_2']['true_abs_error'].median()

        data['test_2']['true_quality_label'] = data['test_2'].apply(lambda row: 1 if row['true_abs_error'] <= test_median_error else 0, axis=1)

        plot_auc(data['test_2'], 'true_quality_label', 'predicted_quality_prob', path=RESULT_PATH + str(exp_num) + '/AUC of classification model.png')
